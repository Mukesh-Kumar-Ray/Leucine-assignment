...
const Home = () => {
  ...
  const arr = [...new Array(totalpage)].map((_,i)=>{return (i+1)});
  ...
  const validateForm = () => {
    ...
    if (!formData.lastCleanedDate)
      newErrors.lastCleanedDate = "Cleaning date is required";
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/equipment?search=${searchTerm}&page=${page}`
        );
        // console.log("mukesh",res);
        setEquipment(res.data.getEquipmentData);
    ...
    fetchAllData();
  }, [formData, editId, searchTerm, refresh,totalpage,page]);
      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-10">
          ...
          <p className="text-gray-600 mt-2">
            Manage your equipments efficiently
          </p>
        </header>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex items-center justify-center gap-2 mt-6">
              {
                page !== 1 && (
                  <button className="px-3 py-1 text-sm border rounded-md text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              
              {
                arr.map((i)=>{
                  return (
                     <button key={i} className={`px-3 py-1 text-sm border rounded-md ${i===page ? "bg-black text-white border-black":"bg-white text-gray-700 hover:bg-gray-100"}`}
                     onClick={()=>setPage(i)}
                     >
                       {i}
                     </button>
